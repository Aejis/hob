(function() {
  ...
  function finder(stor, context) {
    return context.querySelectorAll(stor);
  }

  function matcher(node, stor) {
    return node.matches(stor);
  };
  ...
  DOMMethods = {
    ...
    */
    ...
    find: function(stor) {
      ...
      while (i < len) {
        var found = finder(stor, this[i++]),
            j = 0, ll = found.length;

        while(j < ll) result.push(found[j++]);
      }
